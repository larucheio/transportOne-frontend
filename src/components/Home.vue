<template>
  <div>
    <pricing></pricing>
    <div class="row u-center-text">
      <div class="col-sm-3">
        <img src="../assets/best_price.png" alt="Meilleur prix" height="150" width="150">
        <h5 class="font-weight-bold">Garantie meilleurs prix</h5>
        <p>Faites des économies avec nos prix les plus bas de Genève. Nos tarifs sont fixes et garantis lors de la réservation.</p>
      </div>
      <div class="col-sm-3">
        <img src="../assets/free_cancellation.png" alt="Annulation gratuite" height="150" width="150">
        <h5 class="font-weight-bold">Annulation gratuite</h5>
        <p>Un changement de programme? aucun soucis. Vous disposez d&#39;une annulation gratuite avec un préavis de 24 heure. (par téléphone ou par mail).</p>
      </div>
      <div class="col-sm-3">
        <img src="../assets/service_in_time.png" alt="Service dans les temps" height="150" width="150">
        <h5 class="font-weight-bold">Service dans les temps</h5>
        <p>Grace a notre connaissance minutieuse de Genève et ses environs, nous garantissons un service ponctuelle et dans les temps.</p>
      </div>
      <div class="col-sm-3">
        <img src="../assets/pro_driver.png" alt="Chauffeurs professionels" height="150" width="150">
        <h5 class="font-weight-bold">Chauffeurs professionels</h5>
        <p>Laissez vous transportez sereinement! L&#39;ensemble des chauffeurs Transport One est assuré et agréé. Nous garantissons un service soigné et de qualité.</p>
      </div>
    </div>
    <h1>Nos services</h1>
    <p>Transport One permet à tout le monde d’être transportées et accompagnées durant leurs déplacements, tout en faisant des économies!
      Laissez vous donc transporter sereinement dans notre rayonnante ville de Genève et ses environs.</p>
    <p>Fondé en 1998, Transport One est une entreprise Familiale spécialisée dans le transport accompagné avec chauffeur privé.</p>
    <p>Notre engagement professionnelle garantit un service ponctuelle et de qualité ainsi qu’un accompagnement soigné et sécurisé.</p>
    <p>Que ce soit pour:</p>
    <ul>
      <li>Rendez-vous médicaux</li>
      <li>Centre de bien-être</li>
      <li>Ecole</li>
      <li>Rendez-vous privé</li>
      <li>Toute autre destination</li>
    </ul>
    <p>Et que vous soyez:</p>
    <ul>
      <li>Personnes âgées</li>
      <li>Adultes</li>
      <li>Enfants</li>
      <li>Groupe d’amis, familles, employé d’entreprise,...</li>
      <li>Avec ou sans mobilité réduite (personne en fauteuil roulant non inclus excepté si elle est dans la capacité d’être installé sur l’un de nos siege)</li>
    </ul>
    <p>Nous avons mis en place un service “Commision” qui comprend un aller - retour à domicile tout en accompagnant la personne pour les commissions, la Poste ou autre demande requérant une aide.</p>
    <p>Les destinations “Aéroport” et “Gare” avec accompagnement sont à votre disposition.</p>
  </div>
</template>

<script>
import Pricing from './Pricing.vue'
export default {
  components: { 'pricing': Pricing }
}
</script>

<style>
</style>
