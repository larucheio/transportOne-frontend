<template>
  <div class="container mt-5">
    <h2 class="text-uppercase mb-4">Véhicule et équipement</h2>
    <div class="row py-3">
      <div class="col-md-2 text-center">
        <i class="fa fa-hand-paper-o fa-5x" aria-hidden="true"></i>
      </div>
      <div class="col-md-10">
        <p class="my-4">Certaines entreprises de transport ne respectent pas la loi qui oblige le conducteur à faire une pause et à ne pas travailler plus de certaines heures d’affilées, et cela au dépend du client !</p>
      </div>
    </div>
    <p>Chez Transport One, la durée de conduite <i class="fa fa-car" aria-hidden="true"></i> et la durée de repos <i class="fa fa-battery-quarter" aria-hidden="true"></i> est primordiale et nous l’appliquons à la lettre.</p>
    <div class="row py-3">
      <div class="col-md-6">
        <p>C’est pourquoi afin de respecter minutieusement la loi « OTR2 » sur le Transport Professionnel de Personnes, tous nos véhicules sont équipés de tachygraphe.</p>
      </div>
      <div class="col-md-6">
        <img src="../assets/tachygraphe.jpg" class="img-fluid" alt="tachygraphe">
      </div>
    </div>
    <div class="row py-3">
      <div class="col-md-2 text-center">
        <i class="fa fa-id-card fa-5x" aria-hidden="true"></i>
      </div>
      <div class="col-md-10">
        <p class="my-4">De plus, l&#39;ensemble de nos chauffeurs Transport One sont assurés et agréés.</p>
      </div>
    </div>
    <h5 class="mt-5">Intérieur</h5>
    <ul>
      <li>8 places</li>
      <li>Air climatisé</li>
      <li>Coffre spacieux (Ex : fauteuil roulant pliable)</li>
      <li>Facilité d’accéder au siège</li>
      <li>Equipement de sécurité, poignée de sécurité</li>
      <li>Siège pour bébé</li>
    </ul>
    <div class="row py-3">
      <div class="col-md-6">
        <img src="../assets/car.jpg" class="img-fluid" alt="car">
      </div>
      <div class="col-md-6">
        <img src="../assets/inside-car.jpg" class="img-fluid" alt="inside car">
      </div>
    </div>
    <h1 class="py-5 text-center">Laissez-vous donc transporter sereinement !</h1>
  </div>
</template>
